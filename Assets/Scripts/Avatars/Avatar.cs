...
public class Avatar : MonoBehaviour
{
    ...
    {
        //Get hit
        if (impactObject.name == "Rat_animated" && currentState != "isGettingHit")
        {
            StartGetHit();
        }
        else if (impactObject.name == "Rat_animated (1)" && currentState != "isGettingHit")
        {
            StartGetHit();
        }
        else if (impactObject.name == "Rat_animated (2)" && currentState != "isGettingHit")
        {
            StartGetHit();
        }
        else if (impactObject.name == "Rat_animated (3)" && currentState != "isGettingHit")
        {
            StartGetHit();
        }
        else if (impactObject.name == "white_rat_walking" && currentState != "isGettingHit")
        {
            StartGetHit();
        }
        else if (impactObject.name == "white_rat_walking (1)" && currentState != "isGettingHit")
        {
            StartGetHit();
        }
        else if (impactObject.name == "white_rat_attack" && currentState != "isGettingHit")
        {
            StartGetHit();
        }
    }
    ...
    void RunStates() {
        ...
        //Attack
        if (currentState == "Idle" && attackInput && !myAnimator.IsInTransition(0))
        { StartAttack(); }
        //Run
        else if (currentState != "Running" && currentState != "isGettingHit" && ((runInput || dblTapInput2) && verticalInput > 0))
        { StartRun(); }
        //Walk
        else if (currentState != "Walking" && currentState != "Running" && currentState != "isGettingHit" && (verticalInput != 0 || horizontalInput != 0))
        { StartWalk(); }
        //Idle
    ...
    void StopRun()
    {
        ...
        myAnimator.ResetTrigger("Running");
    }
